<template>
  <div id="X6-1" :class="visible?'full-screen':''">
    <graph-config-container :graph="this.graph" :selectNode="selectNode" @full="handleFull"></graph-config-container>
    <div id="container">
      <div id="stencil"></div>
      <div id="graph-container" ref="graph" @click="hhhhh"></div>
    ...
    methods: {
      ...
      graphOnEvent() {
        ...
        this.graph.on('node:click', ({e, x, y, node, view}) => {
          this.curNode = node
          this.isNode = true
          console.log(this.isNode,"isNode1")
        })
        // 监听画布添加节点动作
        this.graph.on('node:added', ({node, index, options}) => {
          this.curNode = node
          this.isNode = true
          console.log(this.isNode,"isNode2")
        ...
        this.graph.on('edge:added', ({edge, index, options}) => {
          this.isNode = false
          console.log(this.isNode,"isNode4")
          this.curEdge = edge
        ...
        this.graph.on('edge:click', ({e, x, y, edge, view}) => {
          this.isNode = false
          console.log(this.isNode,"isNode3")
          this.curEdge = edge
